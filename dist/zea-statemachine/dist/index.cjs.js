...
class State {
   ...
   */
  constructor(name) {
    this.__name = name ? name : zeaEngine.sgFactory.getClassName(this);
  ...
  toJSON(context, flags) {
    const j = {
      name: this.__name,
      type: zeaEngine.sgFactory.getClassName(this),
    };
  ...
  fromJSON(j, context, flags) {
    ...
    for (const activationActionJson of j.activationActions) {
      const activationAction = zeaEngine.sgFactory.constructClass(activationActionJson.type);
    ...
    for (const deactivationActionJson of j.deactivationActions) {
      const deactivationAction = zeaEngine.sgFactory.constructClass(deactivationActionJson.type);
...
class StateAction extends zeaEngine.ParameterOwner {
   ...
   */
  activate() {
    console.warn('activate must be implmented by each action. this:' + zeaEngine.sgFactory.getClassName(this));
  }
...
zeaEngine.sgFactory.registerClass('SetCameraPositionAndTarget', SetCameraPositionAndTarget);
...
class SetParameterValue extends StateAction {
  ...
  toJSON(context, flags) {
    ...
    if (this.__valueParam) {
      j.valueParamType = zeaEngine.sgFactory.getClassName(this.__valueParam);
    }
    return j
  }
  ...
  fromJSON(j, context, flags) {
    if (j.valueParamType) {
      const param = zeaEngine.sgFactory.constructClass(j.valueParamType, 'Value');
