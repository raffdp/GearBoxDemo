...
const ZeaParamWidgetXfo = class {
    ...
    componentDidLoad() {
        if (this.parameter) {
            this.setUpInputs();
            this.updateDisplayedValue(index_esm.E.USER_SETVALUE);
        }
    }
    /**
    ...
    onInput() {
        ...
        if (!this.change) {
            this.change = new index_esm.N(this.parameter, xfo);
...
UxFactory.uxFactory.registerWidget('zea-param-widget-xfo', (p) => p.constructor.name == index_esm.Se.name);
