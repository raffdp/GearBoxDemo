...
const utils = require('./utils-1b0dfa90.js');
...
const ZeaIcon = class {
    ...
    render() {
        if (this.type == 'ionic') {
            return (index.h("div", { class: "zea-icon ionic" }, index.h("ion-icon", { name: this.name, style: { fontSize: `${this.size}px` } })));
        }
        else if (this.type == 'zea') {
            return (index.h("div", { class: `zea-icon zea-custom-icon zea-icon-${this.name}` }, index.h("div", { class: "zea-custom-icon-inner", style: { fontSize: `${this.size}px` }, innerHTML: getSvg(this.name) })));
        }
    }
};
