import { c as commonjsGlobal, a as createCommonjsModule, b as global } from './global-6e332181.js';
import { p as process } from './index-27446e12.js';
import { E as EventEmitter } from './events-fc0ece3b.js';
...
function isRemote(db) {
    ...
    if (typeof db.type === 'function') {
        guardedConsole('warn', 'db.type() is deprecated and will be removed in ' +
...
Changes$1.prototype.doChanges = function (opts) {
    ...
    if (PouchDB._changesFilterPlugin) {
        ...
        ['doc_ids', 'filter', 'selector', 'view'].forEach(function (key) {
            if (key in opts) {
                guardedConsole('warn', 'The "' + key + '" option was passed in to changes/replicate, ' +
...
AbstractPouchDB.prototype.putAttachment =
    adapterFun('putAttachment', function (docId, attachmentId, rev$$1, blob, type) {
        var api = this;
...
AbstractPouchDB.prototype.removeAttachment =
    adapterFun('removeAttachment', function (docId, attachmentId, rev$$1, callback) {
        var self = this;
...
AbstractPouchDB.prototype.allDocs =
    adapterFun('allDocs', function (opts, callback) {
        ...
        if ('keys' in opts) {
            ...
            if (incompatibleOpt) {
                callback(createError(QUERY_PARSE_ERROR, 'Query parameter `' + incompatibleOpt +
...
AbstractPouchDB.prototype.registerDependentDatabase =
    adapterFun('registerDependentDatabase', function (dependentDb, callback) {
...
function parseIndexableString(str) {
    ...
    while (true) {
        ...
        switch (collationIndex) {
            ...
            /* istanbul ignore next */
            default:
                throw new Error('bad collationIndex or unexpectedly reached end of input: ' +
                    collationIndex);
        }
    }
}
...
function filter(changesHandler, opts) {
    ...
    if (opts.filter === '_view') {
        ...
        changesHandler.db.get('_design/' + viewName[0], function (err, ddoc) {
            ...
            if (!mapFun) {
                return callback(createError(MISSING_DOC, (ddoc.views ? 'missing json key: ' + viewName[1] :
        ...
        changesHandler.db.get('_design/' + filterName[0], function (err, ddoc) {
            ...
            if (!filterFun) {
                return callback(createError(MISSING_DOC, ((ddoc && ddoc.filters) ? 'missing json key: ' + filterName[1]
...
function updateDoc(revLimit, prev, docInfo, results, i, cb, writeDoc, newEdits) {
    ...
    var inConflict = newEdits && (((previouslyDeleted && deleted && merged.conflicts !== 'new_leaf') ||
...
function idbError(callback) {
    return function (evt) {
        ...
        callback(createError(IDB_ERROR, message));
    };
}
...
function idbBulkDocs(dbOpts, req, opts, api, idb, callback) {
    ...
    function verifyAttachment(digest, callback) {
        ...
        req.onsuccess = function (e) {
            if (!e.target.result) {
                var err = createError(MISSING_STUB, 'unknown stub attachment with digest ' +
                    digest);
                err.status = 412;
                callback(err);
            }
...
function idbAllDocs(opts, idb, callback) {
    ...
    if (!keys) {
        ...
        if (keyRangeError &&
            ...
            return callback(createError(IDB_ERROR, keyRangeError.name));
        }
    }
        ...
        if (typeof supportsBulkGet !== 'boolean') {
            ...
            doBulkGet(function (err, res) {
                if (err) {
                    ...
                    explainError(err.status, 'PouchDB is just detecting if the remote ' +
    ...
    function localViewCleanup(db) {
        return db.get('_local/' + localDocName).then(function (metaDoc) {
            ...
            return db.allDocs(opts).then(function (res) {
                ...
                var dbsToDelete = Object.keys(viewsToStatus).filter(function (viewDBName) { return !viewsToStatus[viewDBName]; });
...
function isRemote$1(db) {
    ...
    /* istanbul ignore next */
    if (typeof db.type === 'function') {
        guardedConsole$1('warn', 'db.type() is deprecated and will be removed in ' +
            'a future version of PouchDB');
        return db.type() === 'http';
    }
    /* istanbul ignore next */
    ...
    function queryViewInQueue(view, opts) {
        ...
        if (typeof opts.keys !== 'undefined') {
            ...
            if (typeof endkey !== 'undefined') {
                ...
                viewOpts.endkey = toIndexableString$1(inclusiveEnd ? [endkey, {}] : [endkey]);
            }
            if (typeof opts.key !== 'undefined') {
...
/* localDocName */ 'indexes', mapper$1, reducer$1, ddocValidator$1);
...
function getMultiFieldQueryOpts(selector, index) {
    ...
    for (var i = 0, len = indexFields.length; i < len; i++) {
        ...
        if (!matcher || !Object.keys(matcher).length) { // fewer fields in user query than in index
            ...
            var usingGtlt = ('$gt' in matcher || '$gte' in matcher ||
...
/**
 */
var ZeaWcDataConnector = /** @class */ (function () {
    function ZeaWcDataConnector() {
    }
    /**
     *
     */
    ZeaWcDataConnector.prototype.getDb = function (dataType) {
        var localDB = new PouchDB(dataType, {
    ...
    /**
     */
    ZeaWcDataConnector.prototype.upsertDoc = function (data) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            if (!data || !('type' in data) || !data.type) {
                var err = 'ZeaWcDataConnector: Data must have a type property';
                console.error(err);
                resolve(err);
            }
            var db = _this.getDb(data.type);
            var doPut = function () {
                db.put(data)
                    .then(function (response) {
                    resolve(response);
    ...
    /**
     */
    ZeaWcDataConnector.prototype.getDoc = function (id) {
        var _this = this;
        return new Promise(function (resolve) {
            if (!id) {
                var err = 'ZeaWcDataConnector: Id must be provided';
                console.error(err);
                resolve(err);
            }
            var type = id.split('|', 0);
            var db = _this.getDb(type);
            db.get(id)
                .then(function (doc) {
                resolve(doc);
            })
    ...
    /**
     */
    ZeaWcDataConnector.prototype.deleteDoc = function (id) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            if (!id) {
                var err = 'ZeaWcDataConnector: Id to delete must be provided';
                console.error(err);
                resolve(err);
            }
            var type = id.split('|')[0];
    ...
    /**
     */
    ZeaWcDataConnector.prototype.getDocs = function (options) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var defaults = {
                limit: 10,
            ...
            if (!options.type) {
                var err = 'ZeaWcDataConnector: Type must be provided';
                console.error(err);
                resolve(err);
            }
            var db = _this.getDb(options.type);
            var selector = {};
            db.find({
                selector: selector,
                limit: options.limit,
    ...
    /**
     */
    ZeaWcDataConnector.prototype.destroyDb = function (name) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            _this.getDb(name)
                .destroy()
    ...
    return ZeaWcDataConnector;
}());
export { ZeaWcDataConnector as Z };
