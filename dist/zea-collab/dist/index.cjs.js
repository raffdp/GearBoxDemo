...
var wildcardMiddleware = _interopDefault(require('socketio-wildcard'));
...
class Avatar {
  ...
  constructor(appData, userData, currentUserAvatar = false) {
    ...
    this.__avatarColor = new zeaEngine.Color(userData.color || '#0000ff');
    ...
    if (!this.__currentUserAvatar) {
      ...
      let avatarImage;
      let geom = new zeaEngine.Disc(0.5, 64);
      if (this.__userData.picture && this.__userData.picture != '') {
        avatarImage = new zeaEngine.LDRImage('user' + this.__userData.id + 'AvatarImage');
      ...
      const avatarImageBorderMaterial = new zeaEngine.Material(
        'avatarImageBorderMaterial',
        'FlatSurfaceShader'
      );
      avatarImageBorderMaterial.getParameter('BaseColor').setValue(new zeaEngine.Color(0,0,0,1));
      avatarImageBorderMaterial.visibleInGeomDataBuffer = false;
      const avatarImageBorderGeomItem = new zeaEngine.GeomItem(
        'avatarImageBorder',
        geom,
        avatarImageBorderMaterial
      );

      const borderXfo = new zeaEngine.Xfo();
  ...
  setCameraAndPointerRepresentation() {
    ...
    material.getParameter('BaseColor').setValue(new zeaEngine.Color(0.5, 0.5, 0.5, 1.0));
    ...
    this.__pointerItem = new zeaEngine.GeomItem('Pointer', line, this.__pointermat);
  ...
  updateVRPose(data) {
    ...
    if (data.showUIPanel) {
      if (!this.__uiGeomItem) {
        ...
        this.__uiGeomItem = new zeaEngine.GeomItem('VRControllerUI', this.__plane, uimat);
    ...
    session.sub(Session.actions.USER_JOINED, (userData) => {
      if (!(userData.id in userDatas)) {
        userDatas[userData.id] = {
          undoRedoManager: new zeaUx.UndoRedoManager(),
          avatar: new Avatar(appData, userData),
          selectionManager: new zeaUx.SelectionManager(appData, {
            ...options,
    ...
    if (appData.undoRedoManager) {
      ...
      appData.undoRedoManager.on('changeAdded', (event) => {
        ...
        const data = {
          ...
          changeClass: zeaUx.UndoRedoManager.getChangeClassName(change),
        };
